...
for epoch = 1:nEpochs
    for batch = 1:nBatches
        ...
        trainError(epoch) = trError;
        disp(sprintf('training error after epoch %d: %f\n',epoch,...
        trError));
        vlError = classerror(valY, predict(valydash));
        valError(epoch) = vlError;
...
figure;
plot(1:nEpochs,trainError,'r');
title('Training Error vs the Number of Epoch');
ylabel('Error');
xlabel('Number of Epochs');

figure;
plot(1:nEpochs,valError,'b');
title('Validation Error vs the Number of Epoch');
ylabel('ValidationError');
xlabel('Number of Epochs');
